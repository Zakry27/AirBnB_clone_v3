...
def handle_places(city_id=None, place_id=None):
    '''method handler for places endpoint.
    '''
    handlers = {
        'GET': get_places,
        'DELETE': remove_place,
...
def get_places(city_id=None, place_id=None):
    '''Gets place with given id or all places in
 city with given id.
    '''
    if city_id:
...
def find_places():
    '''Finds places based on list of State, City, or Amenity ids.
    '''
    data = request.get_json()
    if type(data) is not dict:
        raise BadRequest(description='Not a JSON')
